...
class ItemDump:
    ...
    def to_trade_post(self):
        items_section = ''
        sets_str = ''
        for set_ in sorted(self.sets.values(), key=lambda kv: kv[1].name):
            sets_str += f'[u][color=#00FF00]{set_.name}[/color][/u]\n'
            for item in sorted(set_.items.values(), key=lambda kv: kv[1].name):
        ...
        su_str = ''
        for item in sorted(self.su.values(), key=lambda kv: kv[1].name):
        ...
        ssu_str = ''
        for item in sorted(self.ssu.values(), key=lambda kv: kv[1].name):
        ...
        sssu_str = ''
        for item in sorted(self.sssu.values(), key=lambda kv: kv[1].name):
        ...
        runewords_str = ''
        for item in sorted(self.runewords.values(), key=lambda kv: kv[1].name):
        ...
        raqmoj_str = ''
        for item in sorted(self.rings.values(), key=lambda kv: kv[1].name):
            raqmoj_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'

        if self.rings:
            raqmoj_str += '\n'

        for item in sorted(self.amulets.values(), key=lambda kv: kv[1].name):
            raqmoj_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'

        if self.amulets:
            raqmoj_str += '\n'

        for item in sorted(self.quivers.values(), key=lambda kv: kv[1].name):
            raqmoj_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'

        if self.quivers:
            raqmoj_str += '\n'

        for item in sorted(self.mos.values(), key=lambda kv: kv[1].name):
            raqmoj_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'

        if self.mos:
            raqmoj_str += '\n'

        for item in sorted(self.jewels.values(), key=lambda kv: kv[1].name):
            raqmoj_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'
        ...
        bases_str = ''
        for item in sorted(self.rw_bases.values(), key=lambda kv: kv[1].name):
            bases_str += f'[color=#808080]{item.name}[/color] x{item.amount}\n' if item.amount > 1 else f'[color=#808080]{item.name}[/color]\n'

        for item in sorted(self.shrine_bases.values(), key=lambda kv: kv[1].name):
        ...
        charms_str = ''
        for item in sorted(self.charms.values(), key=lambda kv: kv[1].name):
        ...
        trophies_str = ''
        for item in sorted(self.trophies.values(), key=lambda kv: kv[1].name):
        ...
        other_str = ''
        for item in sorted(self.shrines.values(), key=lambda kv: kv[1].name):
            other_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'

        if self.shrines:
            other_str += '\n'

        for item in sorted(self.other.values(), key=lambda kv: kv[1].name):
            other_str += f'[item]{item.name}[/item] x{item.amount}\n' if item.amount > 1 else f'[item]{item.name}[/item]\n'
